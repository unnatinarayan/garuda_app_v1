...
const markAsRead = async (alert) => {
    ...
    try {
        ...
        // 4. CRITICAL REVERT: If API failed, re-add the alert to the local list
        // (You might need to adjust the Pinia store's `addAlert` function to insert it correctly if it was not done at the beginning)
        // Since the current `markAsRead` function is local, we rely on the Pinia store's integrity.
...
<template>
    <div class="notification-container relative inline-block">
        ...
        <button 
            ...
            class="relative transition-colors duration-200 p-2 rounded-full" 
            :class="{'text-white': totalAlerts > 0, 'text-gray-400': totalAlerts === 0}"
            title="View Notifications" style="color: var(--text-muted); background-color: #0f172a;"
        >
        ...
        >
            ...
            <div class="max-h-80 overflow-y-auto">
                ...
                >
                    <div class="flex items-start justify-between p-3">
                        ...
                        <div @click="handleNavigationClick(alert)" class="flex-grow pr-3 cursor-pointer">
                            <p class="text-sm font-bold text-cyan-100 break-words">
                                {{ alert.project_name || alert.projectId }}: {{ alert.aoi_name || alert.aoiId }} has an Alert for {{ alert.algoId }}
                            </p>
                        ...
                        <div class="flex flex-col space-y-2 items-center flex-shrink-0">
                            ...
                            >
                                <svg class="w-4 h-4 transition-transform duration-200" 
                                     :class="{'rotate-180': expandedAlertIds.includes(alert.id)}"
                    ...
                    <div v-if="expandedAlertIds.includes(alert.id)" class="p-3 pt-0 bg-gray-600/50 text-white border-t border-gray-600">
                        <p class="text-xs font-semibold mb-1 text-cyan-200">Raw Message:</p>
                        <pre class="text-xs text-gray-200 whitespace-pre-wrap break-words p-2 bg-gray-700 rounded max-h-40 overflow-y-auto">
