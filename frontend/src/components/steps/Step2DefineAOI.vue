...
const cancelDrawing = () => {
    // 1. Clear the temporary drawing layer from the map
...
const handleCancel = () => {
    // CRITICAL: Clear the temporary drawing layer if the user hits cancel
    if (mapVizRef.value?.clearUnsavedLayer) {
        mapVizRef.value.clearUnsavedLayer();
    }
    showAoiConfigModal.value = false;
...
<template>
    <div>
        <div>
            <MapVisualization @aoi-drawn="handleAoiDrawn" :aois-to-display="projectData.aoiDrafts"
                :is-monitor-mode="false" ref="mapVizRef" />
        </div>
        ...
        <div class="aoi-list-manager space-y-2 max-h-40 overflow-y-auto">
            <div v-for="aoi in projectData.aoiDrafts" :key="aoi.clientAoiId"
                class="aoi-draft flex justify-between items-center p-3 bg-gray-700 rounded shadow-md">
                <span>
                    {{ aoi.name }}
                    <span :class="{ 'text-yellow-400': aoi.bufferDistance }" class="text-sm ml-2">({{ aoi.geometryType
                    }})</span>
                    <span v-if="aoi.bufferDistance" class="text-sm text-green-400 ml-2"> (+{{ aoi.bufferDistance }}m
                        buffer)</span>
                </span>
                <button @click="removeAOI(aoi.clientAoiId)"
                    class="remove-btn bg-red-600 hover:bg-red-700 text-white p-1 rounded">Remove</button>
            </div>
            <p v-if="projectData.aoiDrafts.length === 0" class="text-center text-gray-400 p-4">Draw an AOI on the map
                above to begin.</p>
        </div>
            ...
            <div class="w-full max-w-md bg-gray-800 rounded-xl shadow-2xl p-6 text-white">
                ...
                <div class="space-y-4">
                    <div class="form-group">
                        ...
                        <input type="text" v-model="currentAoiName" placeholder="Enter a unique name" required
                    ...
                    <div v-if="requiresBuffer" class="form-group">
                        ...
                        <input type="number" v-model.number="currentAoiBuffer" min="0" placeholder="0" required
                            class="w-full p-2 bg-gray-700 text-white rounded border border-gray-600">
                    ...
                    <button @click="addAuxField"
                        class="text-white hover:text-blue-300 bg-cyan-600 text-sm mt-2 flex items-center">
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <button @click="showAoiConfigModal = false && handleCancel && removeAOI"
                        class="px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded-lg">Cancel</button>
                    <button @click="finalizeAOI"
                        class="px-4 py-2 bg-cyan-600 hover:bg-cyan-700 rounded-lg font-semibold">Save AOI</button>
                </div>
            </div>
        </div>
    </div>
